...
.drop-zone-view:hover {
    background-color: #f3f4f6;
    border-color: #3b82f6;
}

.drop-zone-view.drag-over {
    background-color: #eff6ff;
    border-color: #3b82f6;
    border-style: solid;
}

.drop-zone-view .drop-zone-icon {
    font-size: 32px;
    color: #3b82f6;
    margin-bottom: 8px;
}

.drop-zone-view .drop-zone-text {
    color: #374151;
    font-size: 13px;
    font-weight: 500;
    margin-bottom: 3px;
...
.drop-zone-view .browse-link {
    color: #3b82f6;
    cursor: pointer;
    text-decoration: underline;
}

.drop-zone-view .drop-zone-hint {
    color: #9ca3af;
    font-size: 11px;
}
...
.upload-file-preview-item .file-icon {
    color: #6b7280;
    font-size: 16px;
}

.upload-file-preview-item .file-name {
    flex: 1;
    color: #374151;
    white-space: nowrap;
    overflow: hidden;
...
.upload-file-preview-item .file-size {
    color: #9ca3af;
    font-size: 11px;
}

.upload-file-preview-item .remove-file-btn {
    background: #ef4444;
    color: white;
    border: none;
    border-radius: 3px;
...
.upload-file-preview-item .remove-file-btn:hover {
    background: #dc2626;
}
            ...
            <div class="container-fluid fluid-top p-3">
                    ...
                    <div class="signal-header-card" style="border-left-color: <?php echo $signal['priority_color']; ?>">
                        ...
                        <div class="signal-meta-row">
							...
							<?php 
							// Set badge colors based on signal type
							$badge_bg = '#f3f4f6'; // default gray background
							$badge_color = '#6b7280'; // default gray text

							if ($signal['signal_type'] === 'project') {
							    $badge_bg = '#f3ecfc';   // light purple
							    $badge_color = '#7c3aed'; // dark purple
							}

							if ($signal['signal_type'] === 'general_request') {
							    $badge_bg = '#ccfbf1';   // light teal
							    $badge_color = '#0d9488'; // dark teal
							}
							...
							?>
							<span class="meta-badge" style="background: <?php echo $badge_bg; ?>; color: <?php echo $badge_color; ?>;" 
							    data-bs-toggle="tooltip" data-bs-placement="top" title="Type">
							    <i class="fa-solid <?php echo ($signal['signal_type'] === 'general_request') ? 'fa-layer-group' : (($signal['signal_type'] === 'project') ? 'fa-diagram-project' : 'fa-circle-question'); ?>"></i>
							    <?php echo $type_labels[$signal['signal_type']] ?? 'Unknown'; ?>
							</span>
                        </div>
                        ...
                        <div class="shareable-link">
                            ...
                            <span style="font-size: 0.875rem; color: #6b7280; margin-left: 8px; font-weight: 500;">Keeper View:</span>
                            <input type="text" id="signalShareLink" readonly 
                                   value="<?php echo htmlspecialchars('https://' . $_SERVER['HTTP_HOST'] . '/dashboard/lighthouse_keeper_view.php?id=' . $signal_id, ENT_QUOTES, 'UTF-8'); ?>">
                            <button type="button" class="btn-copy-link" onclick="copyShareLink('signalShareLink')">
                                <i class="fa-solid fa-copy"></i> Copy
                            </button>
                        </div>
                        
                        <div class="shareable-link" style="margin-top: 10px;">
                            <i class="fa-solid fa-link" style="color: #6b7280;"></i>
                            <span style="font-size: 0.875rem; color: #6b7280; margin-left: 8px; font-weight: 500;">Harbor View:</span>
                            <input type="text" id="signalHarborShareLink" readonly 
                            ...
                            <button type="button" class="btn-copy-link" onclick="copyShareLink('signalHarborShareLink')">
                                <i class="fa-solid fa-copy"></i> Copy
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="row gx-3">
                        ...
                        <div class="col-lg-4">
							...
							<!-- Attachments Section with Upload Capability -->
							<div class="content-section">
							    <div class="section-title" style="display: flex; align-items: center; justify-content: space-between;">
							        ...
							        <button type="button" 
							                ...
							                style="background: #10b981; color: white; border: none; padding: 6px 12px; border-radius: 6px; font-size: 12px; cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; gap: 6px;">
												...
												<div class="attachment-info">
												                        ...
												                        <div class="attachment-meta">
												                            <div class="attachment-uploader">
												                                ...
												                                <span class="attachment-uploader-name">
												                                    <?php echo $attachment['uploader_name'] ? htmlspecialchars($attachment['uploader_name']) : 'Unknown'; ?>
												                                </span>
												                            </div>
												                            <span class="attachment-date"><?php echo date('M d, Y g:i A', strtotime($attachment['uploaded_date'])); ?></span>
												                        </div>
												                    </div>
							                </div>
							            </div>
							    ...
							    <!-- Upload Section for Adding More Files -->
							    <?php if ($is_admin): ?>
							    <div class="upload-section" id="upload-section-container" style="display: none; opacity: 0; transition: opacity 0.3s ease; margin-top: 15px;">
							        <div class="upload-area-title">
							            <i class="fa-solid fa-cloud-arrow-up"></i>
							            Add More Files
							        </div>
							        <div class="drop-zone-view" id="additional-drop-zone">
							            <input type="file" class="file-input-hidden" id="additional-file-input" accept=".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.txt,.csv,.xls,.xlsx" multiple>
							            <div class="drop-zone-icon">
							                <i class="fa-solid fa-cloud-arrow-up"></i>
							            </div>
							            <div class="drop-zone-text">
							                Drag & Drop or <span class="browse-link" onclick="document.getElementById('additional-file-input').click()">browse</span>
							            </div>
							        ...
							        <div class="upload-files-preview" id="additional-files-preview">
							            <!-- Files to upload will appear here -->
							        </div>
        
							        <div class="upload-actions">
							            <button type="button" class="btn-upload-files" id="btn-upload-additional-files" onclick="uploadAdditionalFiles()" disabled>
							                <i class="fa-solid fa-upload"></i> Upload <span class="file-count">0</span> File(s)
							            </button>
							            <button type="button" class="btn-clear-files" onclick="clearAdditionalFiles()">
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" onclick="viewAttachmentInNewTab()">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <!-- Debug: <?php echo "Current signal_type value: [" . $signal['signal_type'] . "]"; ?> -->
                            <select name="signal_type" class="form-select" required>
                                <option value="">Select Type</option>
                                <option value="general_request" <?php echo ($signal['signal_type'] == 'general_request') ? 'selected' : ''; ?>>General</option>
                                <option value="project" <?php echo ($signal['signal_type'] == 'project') ? 'selected' : ''; ?>>Project</option>
                                <option value="other" <?php echo ($signal['signal_type'] == 'other') ? 'selected' : ''; ?>>Other</option>
                            </select>
                        </div>
...
<script src="https://cdn.jsdelivr.net/npm/tinymce@5.10.5/tinymce.min.js" referrerpolicy="origin"></script>
<!-- https://cdn.tiny.cloud/1/6i7udj9tuqovoj6lp5jpkopu2phxpzqoe6g35gx49wbr3v1u/tinymce/7/tinymce.min.js -->
...
$(document).ready(function() {
	...
	var editSignalModal = document.getElementById('editSignalModal');
    var tinyMCEInitialized = false;
    
    if (editSignalModal) {
        editSignalModal.addEventListener('shown.bs.modal', function() {
    	...
    	editSignalModal.addEventListener('hidden.bs.modal', function() {
            if (tinyMCEInitialized && tinymce.get('edit_keeper_signal_message_content')) {
                tinymce.get('edit_keeper_signal_message_content').remove();
	...
	$('#updateType').on('change', function() {
 	...
 	$(document).on('click', '.delete-update-comment-btn', function() {
  	...
  	$(document).on('click', '.cancel-update-edit-btn', function() {
        ...
        updateItem.find('.update-item-actions').show().css('opacity', '');
...
function viewAttachment(filePath, fileName, isImage, attachmentId) {
    ...
    currentAttachmentPath = filePath; // Store the file path
...
function viewAttachmentInNewTab() {
    if (!currentAttachmentPath) {
        alert('Unable to determine attachment path');
        return;
    }
...
function setupFileUpload() {
    const dropZone = $('#additional-drop-zone')[0];
    const fileInput = $('#additional-file-input')[0];
...
function handleAdditionalFiles(files) {
    ...
    for (let file of files) {
        ...
        if (!allowedTypes.includes(file.type)) {
            alert(`File "${file.name}" has an unsupported format. Allowed types: JPG, PNG, GIF, WEBP, PDF, DOC, DOCX, TXT, CSV, XLS, XLSX.`);
            continue;
        }

        additionalFiles.push(file);
        displayAdditionalFile(file, additionalFiles.length - 1);
    }

    updateUploadButton();
}

function displayAdditionalFile(file, index) {
    ...
    const fileItem = $(`
        <div class="upload-file-preview-item" data-file-index="${index}">
            <i class="fa-solid ${fileIcon} file-icon"></i>
            <span class="file-name">${file.name}</span>
            <span class="file-size">${fileSize}</span>
            <button type="button" class="remove-file-btn" onclick="removeAdditionalFile(${index})">
                <i class="fa-solid fa-times"></i>
            </button>
        </div>
    `);

    $('#additional-files-preview').append(fileItem);
}

function removeAdditionalFile(index) {
    additionalFiles.splice(index, 1);
    $(`.upload-file-preview-item[data-file-index="${index}"]`).remove();
    updateUploadButton();
}

function clearAdditionalFiles() {
    additionalFiles = [];
    $('#additional-files-preview').html('');
    updateUploadButton();
}

function updateUploadButton() {
    const btn = $('#btn-upload-additional-files');
    if (additionalFiles.length > 0) {
        btn.prop('disabled', false);
        btn.find('.file-count').text(additionalFiles.length);
    } else {
        btn.prop('disabled', true);
        btn.find('.file-count').text('0');
    }
}
...
function toggleUploadSection() {
    ...
    if (isVisible) {
        ...
        // Update button
        toggleBtn.innerHTML = '<i class="fa-solid fa-plus"></i><span>Add Files</span>';
        ...
        // Update button
        toggleBtn.innerHTML = '<i class="fa-solid fa-minus"></i><span>Hide Upload</span>';
...
function uploadAdditionalFiles() {
    ...
    formData.append('signal_id', <?php echo $signal_id; ?>);

    for (let i = 0; i < additionalFiles.length; i++) {
        formData.append('signal_attachments[]', additionalFiles[i]);
    }

    $.ajax({
        url: 'ajax/lh_attachments/add_signal_attachments.php',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(response) {
            if (response.success) {
                alert(`Successfully uploaded ${response.attachments_uploaded} file(s)!`);
