...
if (!empty($_POST["response_answer"])) {
    $query = "INSERT INTO poll_response (question_id, response_type, response_key, response_answer, response_info) VALUES (?, ?, ?, ?, ?)";
    
	if ($stmt = mysqli_prepare($dbc, $query)) {
    	foreach ($_POST["response_answer"] as $key => $value) {
            $question_id = $_POST['question_id'][$key];
            $response_type = $_POST['response_type'][$key];
            ...
            $response_info = $_POST['response_info'][$key];
          	...
          	mysqli_stmt_bind_param($stmt, 'ssiss', $question_id, $response_type, $response_key, $response_answer, $response_info);
