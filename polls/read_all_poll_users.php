...
$(document).ready(function(){
	function countPollUserRecords(){
		var count = $('.count-poll-user-item').length;
	    $('.count-poll-user-records').html(count);
	} countPollUserRecords();
});
</script>
	...
	if (isset($_POST['inquiry_id']) && $_POST['inquiry_id'] !== ""){
		...
		$data ='<script>
				$(document).ready(function(){
					...
					$("input:checkbox").click(function() {
						if ($(this).is(":checked")) {
							$(".hide_and_seek").prop("disabled", false);
							 ...
							 if ($(".chk-box-poll-user-select").filter(":checked").length < 1){
				 ...
				 <script>
				 $("#all_poll_users_table").DataTable({
					 aLengthMenu: [
					 	[100, 200, -1],
						[100, 200, "All"]
					 ],
		...
		$data .='<div class="row">
					<div class="col-sm-4 mb-3">
						<select class="form-select hidden_poll_user_select" id="poll-user-action-select">
							<option value="">Select an Action</option>
							<option value="enroll_poll_user">Enroll</option>
							<option value="remove_poll_user">Remove</option>
						</select>
					</div>
					<div class="col-sm-6 mb-3">
						<button type="button" class="btn btn-outline-secondary hidden_poll_user_select me-1" onclick="applyPollUserAction('.htmlspecialchars($inquiry_id).');">
							Apply <small>( <span class="my_poll_users_count">0</span> )</small>
						</button>
					</div>
				</div>';
					
		$data .='<div class="table-responsive p-1">
					<table class="table table-sm" id="all_poll_users_table" width="100%">
						<thead class="bg-light">
							<th> 
								<div class="form-check form-switch">
									<input type="checkbox" class="form-check-input select-all-poll-users" id="select-all-poll-users">
									<label class="form-check-label" for="select-all-poll-users"></label>
								</div>
							</th>
							<th>Photo</th>
							<th>Name</th>
		...
		if ($stmt = mysqli_prepare($dbc, $query)) {
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				$data .='<tr id="user_info">
								...
								<div class="form-check form-switch">
									<input type="checkbox" class="form-check-input chk-box-poll-user-select" data-poll-user-id="'.htmlspecialchars($inquiry_id).'" data-emp-user="'.htmlspecialchars($user).'" data-emp-fname="">
					    			<label class="form-check-label" for=""></label>
								</div>
							</td>
							
							<td class="align-middle" style="width:5%;"><img src="' . htmlspecialchars($profile_pic) . '" class="profile-photo"></td>
				...
				$query_two = "SELECT * FROM poll_assignment WHERE assignment_status = 'Yes' AND poll_id = ? AND assignment_user = ?";
				if ($stmt_two = mysqli_prepare($dbc, $query_two)) {
					mysqli_stmt_bind_param($stmt_two, "ss", $inquiry_id, $user);
					...
					if (mysqli_num_rows($result_two) > 0) {
						while ($row_two = mysqli_fetch_assoc($result_two)) {
							$poll_name = htmlspecialchars(strip_tags($row_two['poll_name']));
				...
				$query_three = "SELECT * FROM poll_ballot WHERE question_id = ? AND ballot_user = ?";
				if ($stmt_three = mysqli_prepare($dbc, $query_three)) {
					mysqli_stmt_bind_param($stmt_three, "ss", $inquiry_id, $user);
					mysqli_stmt_execute($stmt_three);
					$result_three = mysqli_stmt_get_result($stmt_three);
				...
				$query_four = "SELECT * FROM poll_inquiry WHERE inquiry_id = ?";
				if ($stmt_four = mysqli_prepare($dbc, $query_four)) {
					mysqli_stmt_bind_param($stmt_four, "s", $inquiry_id);
					mysqli_stmt_execute($stmt_four);
					$result_four = mysqli_stmt_get_result($stmt_four);
					confirmQuery($result_four);
					
					while ($row_four = mysqli_fetch_array($result_four)) {
						$inquiry_status = htmlspecialchars(strip_tags($row_four['inquiry_status']));	
...
<script>
$(document).ready(function() {
	var $audit_chkboxes = $(".chk-box-poll-user-select");
	...
	$('.select-all-poll-users').on('click', function(e) {
 	 	 if ($(this).is(':checked',true)) {
			 $(".chk-box-poll-user-select").prop('checked', true);
			 var count = $(".chk-box-poll-user-select:checked").length;
 			 var count_zero = '0';
			 if (count != 0){
 				 $(".my_poll_users_count").html(count);
 			 } else {
 				 $(".my_poll_users_count").html(count_zero);
 			 }
		 } else { 
			 $(".chk-box-poll-user-select").prop('checked',false);
			 var count = '0';
 			 var count_zero = '0';
			 if (count != 0){
 				 $(".my_poll_users_count").html(count);
	...
	$(".chk-box-poll-user-select").on('click', function(e) {
		var count = $(".chk-box-poll-user-select:checked").length;
		var count_zero = '0';
		...
		if ($(this).is(':checked',true)) {
			$(".select-all-poll-users").prop("checked", false);
		} else {
			$(".select-all-poll-users").prop("checked", false);
			window.history.pushState({},'','poll_manager.php');
		}

		if ($(".chk-box-poll-user-select").not(':checked').length == 0) {
			$(".select-all-poll-users").prop("checked", true);
		}
	}); 
	
	$('.hidden_poll_user_select').prop("disabled", true);
	
	if ($('.chk-box-poll-user-select').is(':checked',true)) {
		$('.hidden_poll_user_select').prop("disabled", false);
	} else {
		$('.hidden_poll_user_select').prop("disabled", true);
	}

	$('#all_poll_users_table').on("change", ".chk-box-poll-user-select", function(event) {
		if ($('.chk-box-poll-user-select').is(':checked',true)) {
			$('.hidden_poll_user_select').prop("disabled", false);
		} else {
			$('.hidden_poll_user_select').prop("disabled", true);
	...
	$('#all_poll_users_table').on("change", ".select-all-poll-users", function(event) {
		if ($('.select-all-poll-users').is(':checked',true)) {
			$('.hidden_poll_user_select').prop("disabled", false);
		} else {
			$('.hidden_poll_user_select').prop("disabled", true);
		}
...
<script>
$(document).ready(function(){
	$("#inquiry_poll_image_link_edit").change(function () {
		$('#inquiry_poll_image_preview_edit').attr('src', $('#inquiry_poll_image_link_edit').val());
	}) 
	$(".reset_poll_edit_link_image").click(function() {
		$('#inquiry_poll_image_link_edit').val('');
		$('#inquiry_poll_image_preview_edit').attr('src', 'media/links/default_poll_image.png');
	});
});
</script>