...
if (isset($_SESSION['id'])) {
	...
	$(document).ready(function(){
		$(".answer_move_icon").mousedown(function(){
	 		$("#sortable_create_range_slider_row").sortable({
				axis: "y",
    ...
    $data .= '<form name="create_range_slider_form" id="create_range_slider_form">';
    ...
    if ($stmt = mysqli_prepare($dbc, $query)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $data .= '<div class="table-responsive">
                        <table class="table mb-3" id="add_new_range_slider_row">
                            ...
                            <tbody class="bg-white" id="sortable_create_range_slider_row">
                                <tr> 
                    ...
                    <div class="row g-2"> 
                        <div class="col-md-8">
                            <button type="button" class="btn btn-primary btn-lg w-100 shadow-sm" name="submit" id="create-poll-range-slider-answers"><i class="fa-regular fa-circle-check"></i> Create Answers</button>
                        </div> 
                        <div class="col-md-4">
                        	<button type="button" class="btn btn-light-gray btn-lg w-100 shadow-sm" name="add-poll-range-slider-answer" id="add-poll-range-slider-answer"><i class="fa-solid fa-circle-plus"></i> Row</button>
                        </div>
                    </div>';
...
$(document).ready(function(){ 
	...
	$('#add-poll-range-slider-answer').click(function(){  
		$("#sortable_create_range_slider_row").sortable({
			axis: "y",
			helper: function(e, tr)
			  {
	...
	$('#create-poll-range-slider-answers').click(function(){
		...
		if (response_answer_text.length === 0) {
			...
			$("#create-poll-range-slider-answers").prop("disabled", true);
			$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Saving...');
		
			$.ajax({ 
				method:"POST",
				url: "ajax/polls/create_poll_range_slider.php",
				data: $('#create_range_slider_form').serialize(), 
				type:'json',
				success:function(data){
					i = 1;
					$('.dynamic-added-row').remove();
					$('#create_range_slider_form')[0].reset();
					$("#create-poll-range-slider-answers").prop("disabled", false);
					$("#create-poll-range-slider-answers").html('<i class="fa-solid fa-cloud"></i> Save');
