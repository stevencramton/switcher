...
if (isset($_SESSION['id'])) {
	...
	$(document).ready(function(){
        $(".answer_move_icon").mousedown(function(){
            $("#sortable_create_star_rating_row").sortable({
                axis: "y",
    ...
    $data .= '<form name="create_star_rating_form" id="create_star_rating_form">';
    ...
    if ($stmt) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $data .= '<div class="table-responsive">
                <table class="table mb-3" id="add_new_star_rating_row">
                    ...
                    <tbody class="bg-white" id="sortable_create_star_rating_row">
                        <tr>
            ...
            <div class="row g-2">
                <div class="col-md-8">
                    <button type="button" class="btn btn-primary btn-lg w-100 shadow-sm" name="submit" id="create-poll-star-rating-answers"><i class="fa-regular fa-circle-check"></i> Create Answers</button>
                </div>
                <div class="col-md-4">
                   <button type="button" class="btn btn-light-gray btn-lg w-100 shadow-sm" name="add-poll-star-rating-answer" id="add-poll-star-rating-answer">
				   		<i class="fa-solid fa-circle-plus"></i> Row
					</button>
                </div>
            </div>';
        } else {
...
$(document).ready(function(){ 
	...
	$('#add-poll-star-rating-answer').click(function(){  
		$("#sortable_create_star_rating_row").sortable({
			axis: "y",
			helper: function(e, tr)
			  {
	...
	$('#create-poll-star-rating-answers').click(function(){
		...
		if (response_answer_text.length === 0) {
			...
			$("#create-poll-star-rating-answers").prop("disabled", true);
			$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Saving...');
		
			$.ajax({ 
				method:"POST",
				url: "ajax/polls/create_poll_star_rating.php",
				data: $('#create_star_rating_form').serialize(), 
				type:'json',
				success:function(data){
					i = 1;
					$('.dynamic-added-row').remove();
					$('#create_star_rating_form')[0].reset();
					$("#create-poll-star-rating-answers").prop("disabled", false);
					$("#create-poll-star-rating-answers").html('<i class="fa-solid fa-cloud"></i> Save');
