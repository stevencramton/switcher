...
if (isset($_SESSION['id'])) {
	...
	$(document).ready(function(){
		$(".answer_move_icon").mousedown(function(){
	 		$("#sortable_create_true_false_row").sortable({
				axis: "y",
	...
	$data .= '<form name="create_true_false_form" id="create_true_false_form">';
	...
	if (mysqli_num_rows($result) > 0) {
		...
		$data .= '<div class="table-responsive">
			<table class="table mb-3" id="add_new_true_false_row">
				...
				<tbody class="bg-white" id="sortable_create_true_false_row">
					<tr> 
		...
		<div class="row g-2"> 
			<div class="col-md-8">
				<button type="button" class="btn btn-primary btn-lg w-100 shadow-sm" name="submit" id="create-poll-true-false-answers"><i class="fa-regular fa-circle-check"></i> Create Answers</button>
			</div> 
			<div class="col-md-4">
				<button type="button" class="btn btn-light-gray btn-lg w-100 shadow-sm" name="add-poll-true-false-answer" id="add-poll-true-false-answer"><i class="fa-solid fa-circle-plus"></i> Row</button>
			</div>
		</div>';
	} else {
...
$(document).ready(function(){ 
	...
	$('#add-poll-true-false-answer').click(function(){  
		$("#sortable_create_true_false_row").sortable({
			axis: "y",
			helper: function(e, tr)
			  {
	...
	$('#create-poll-true-false-answers').click(function(){
		...
		if (response_answer_text.length === 0) {
			...
			$("#create-poll-true-false-answers").prop("disabled", true);
			$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Saving...');
		
			$.ajax({ 
				method:"POST",
				url: "ajax/polls/create_poll_true_false.php",
				data: $('#create_true_false_form').serialize(), 
				type:'json',
				success:function(data){
					i = 1;
					$('.dynamic-added-row').remove();
					$('#create_true_false_form')[0].reset();
					$("#create-poll-true-false-answers").prop("disabled", false);
					$("#create-poll-true-false-answers").html('<i class="fa-solid fa-cloud"></i> Save');
