...
$data = '<select class="form-select" name="newRoleTypeCat" id="newRoleTypeCat">';
...
if ($stmt = mysqli_prepare($dbc, $role_cat_query)) {
    ...
    while ($role_cat_row = mysqli_fetch_assoc($result)) {
        ...
        $data .= '<option value="' . htmlspecialchars($role_cat_id) . '">' . htmlspecialchars($role_cat_name) . '</option>';
    }
