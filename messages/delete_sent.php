...
if (isset($_SESSION['id']) && isset($_POST['message_sent_id'])) {
    ...
    $message_sent_id = $_POST['message_sent_id'];
	$message_ids = explode(',', $message_sent_id);
	$placeholders = implode(',', array_fill(0, count($message_ids), '?'));

    $query = "DELETE FROM messages_sent WHERE sender = ? AND id IN ($placeholders)";
    $stmt = mysqli_prepare($dbc, $query);
    if ($stmt === false) {
        die('Error.');
    }

	$types = 's' . str_repeat('i', count($message_ids));
	$params = array_merge([$types, $msg_user_full], $message_ids);
