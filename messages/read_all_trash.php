...
$(document).ready(function(){
	function countAllTrashMessages(){
		var count_all_trash = $(".count-all-trash-items").length;
		$(".count-all-trash-messages").html(count_all_trash);
	} countAllTrashMessages();
});
</script>
...
if (isset($_SESSION['id'])) {
    ...
    $data = '<script>
	$(document).ready(function(){
	$(".hide_and_seek_all_trash").prop("disabled", true);
	$("input:checkbox").click(function() {
		if ($(this).is(":checked")) {
			$(".hide_and_seek_all_trash").prop("disabled", false);
		} else {
  		  	if ($(".chk-box-trash-all").filter(":checked").length < 1){
     		   	$(".hide_and_seek_all_trash").attr("disabled",true);
			}
		}
	});
});
...
$("#all_trash_table").dataTable({
 ...
 aLengthMenu: [
    ...
    $data .= '<div class="table-responsive p-1">
                <table class="table table-sm" id="all_trash_table">
    ...
    if (checkRole('messages_restore') || checkRole('messages_delete')) {
        $data .= '<th>
                    <div class="form-check form-switch">
                        <input type="checkbox" class="form-check-input select-all-trash" id="switch22">
                        <label class="form-check-label" for="switch22"></label>
                    </div>
                  </th>';
    }
	...
	$query = "SELECT * FROM messages WHERE trash = 1 ORDER BY id DESC";
    ...
    if ($stmt = mysqli_prepare($dbc, $query)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $data .= '<tr class="count-all-trash-items ';
            ...
            if (checkRole('messages_restore') || checkRole('messages_delete')) {
                $data .= '<td class="align-middle" style="width:3%;">
                            <div class="form-check form-switch">
                                <input type="checkbox" class="form-check-input chk-box-trash-all" data-message-id="'.$id.'">
...
$(document).ready(function() {
	$('.select-all-trash').on('click', function(e) {
 	 	 if($(this).is(':checked',true)) {
			 $(".chk-box-trash-all").prop('checked', true);
		 } else {
			 $(".chk-box-trash-all").prop('checked',false);
		 }
	});
	$(".chk-box-trash-all").on('click', function(e) {
		if($(this).is(':checked',true)) {
		...
		if ($(".chk-box-trash-all").not(':checked').length == 0) {
			$(".select-all-trash").prop("checked", true);
		}
	});
	
	$('.hide_and_seek_all_trash').prop("disabled", true);
	$('#all_trash_table').on("click", 'input:checkbox', function() {
		if ($(this).is(':checked')) {
			$('.hide_and_seek_all_trash').prop("disabled", false);
		} else {
			if ($('.chk-box-trash-all').filter(':checked').length < 1){
