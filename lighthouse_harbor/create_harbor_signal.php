...
// Set signal_type - default to 'general_request' for harbor users
$signal_type = !empty($_POST['signal_type']) ? trim($_POST['signal_type']) : 'general_request';

// Validate signal_type against allowed values
$allowed_signal_types = ['general_request', 'project', 'other'];
if (!in_array($signal_type, $allowed_signal_types)) {
    $signal_type = 'general_request'; // Fallback to default if invalid
}
...
$insert_query = "INSERT INTO lh_signals (signal_number, title, message, signal_type, priority_id, sea_state_id, dock_id, sent_by, sent_date, updated_date) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$insert_stmt = mysqli_prepare($dbc, $insert_query);
mysqli_stmt_bind_param($insert_stmt, 'ssssiiiss', $signal_number, $title, $message, $signal_type, $priority_id, $sea_state_id, $dock_id, $user_id, $current_datetime, $current_datetime);
