...
// CRITICAL: Filter out closed resolution signals unless specifically filtering for closed
// If owner is 'closed', show ONLY closed signals
// If a specific state is selected, show signals for that state
// Otherwise, exclude closed resolution signals from the view
if ($owner === 'closed') {
...
if ($owner === 'my_signals') {
    ...
    // For closed filter, show user's own closed signals
    $query .= " AND t.sent_by = ?";
    $params[] = $user_id;
    $types .= 'i';
...
// Skip this check if owner is 'closed' since we already added sent_by filter above
if (!$is_admin && $owner !== 'all' && $owner !== 'closed' && $owner !== 'my_signals') {
    $query .= " AND t.sent_by = ?";
    $params[] = $user_id;
    $types .= 'i';
}
...
function getCounts($dbc, $user_id, $is_admin) {
    ...
    // My CLOSED signals count - for Harbor users sidebar
    $query = "SELECT COUNT(*) as count 
              FROM lh_signals t
              LEFT JOIN lh_sea_states ts ON t.sea_state_id = ts.sea_state_id
              WHERE t.is_deleted = 0 
