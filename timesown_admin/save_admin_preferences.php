...
mysqli_stmt_bind_param($stmt, 'i', $preferred_tenant_id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);

if (mysqli_num_rows($result) === 0) {
    echo json_encode(['success' => false, 'message' => 'Invalid organization - tenant not found or inactive']);
    exit();
}
...
try {
    $create_table_query = "
        CREATE TABLE IF NOT EXISTS `to_admin_user_preferences` (
    ...
    $upsert_query = "
        INSERT INTO to_admin_user_preferences (user_id, preferred_tenant_id) 
        VALUES (?, ?) 
