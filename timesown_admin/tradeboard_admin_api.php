...
function approveTrade($dbc, $admin_user_id, $input) {
    ...
    try {
        ...
        mysqli_autocommit($dbc, true);
     	...
     	error_log('Approve Trade - Exception: ' . $e->getMessage() . ' in ' . $e->getFile() . ' on line ' . $e->getLine() . ' | Trade ID: ' . ($trade_id ?? 'unknown'));
        
      	ob_end_clean();
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to approve trade']);
    }
}
...
function rejectTrade($dbc, $admin_user_id, $input) {
    ...
    try {
        ...
        mysqli_autocommit($dbc, true);
      	...
      	error_log('Reject Trade - Exception: ' . $e->getMessage() . ' in ' . $e->getFile() . ' on line ' . $e->getLine() . ' | Trade ID: ' . ($trade_id ?? 'unknown'));
     	ob_end_clean();
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to reject trade']);
    }
}
