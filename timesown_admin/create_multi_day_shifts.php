...
try {
    foreach ($shift_dates as $shift_date) {
       	if ($assigned_user_id) {
            ...
            if (!$stmt) {
             	error_log("Multi-Day Shifts - Failed to prepare conflict query for $shift_date: " . mysqli_error($dbc));
             	$errors[] = "Failed to check conflicts for $shift_date";
                $skipped_count++;
                continue;
            }
            ...
            if (!mysqli_stmt_execute($stmt)) {
             	error_log("Multi-Day Shifts - Failed to execute conflict query for $shift_date: " . mysqli_stmt_error($stmt));
              	$errors[] = "Failed to check conflicts for $shift_date";
                $skipped_count++;
                continue;
            }
        ...
        if (!$stmt) {
          	error_log("Multi-Day Shifts - Failed to prepare insert query for $shift_date: " . mysqli_error($dbc));
        	$errors[] = "Failed to create shift for $shift_date";
            $skipped_count++;
            continue;
        }
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if ($audit_stmt) {
          	...
          	error_log("Multi-Day Shifts - Failed to create shift for $shift_date: " . mysqli_error($dbc) . " | Stmt error: " . mysqli_stmt_error($stmt));
    ...
    if ($created_count === 0) {
        ...
        echo json_encode([
            'success' => false, 
            'message' => 'No shifts could be created',
            'details' => $errors
        ]);
        exit();
    }
    ...
    mysqli_autocommit($dbc, true);
 	...
 	error_log('Multi-Day Shifts Creation Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ' on line ' . $e->getLine());
	http_response_code(500);
    echo json_encode([
        'success' => false,
        'message' => 'An error occurred while creating shifts'
    ]);
}
?>