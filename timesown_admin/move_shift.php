...
if ($new_role_id !== null && $new_role_id != $current_shift['job_role_id']) {
    ...
    if (mysqli_num_rows($result) === 0) {
    	error_log(sprintf(
            "Invalid role for department [User: %d, Role: %d, Dept: %d, New Dept: %d, Current Dept: %d]",
            $user_id,
            $new_role_id,
            $dept_id_for_validation,
            $new_department_id ?? 0,
            $current_shift['department_id']
        ));
...
try {
    ...
    if (!mysqli_stmt_execute($stmt)) {
        ...
        error_log(sprintf(
            "Database error in shift move [User: %d, Shift: %d]: %s",
            $user_id,
            $shift_id,
            $db_error
        ));
        throw new Exception('Database error updating shift');
    }
    ...
    mysqli_autocommit($dbc, true);
 	...
 	error_log(sprintf(
        "Move Shift Error [User: %d, Shift: %d, Tenant: %d]: %s in %s:%d",
        $user_id ?? 0,
        $shift_id ?? 0,
        $tenant_id ?? 0,
        $e->getMessage(),
