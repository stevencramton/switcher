...
try {
    switch ($method) {
        ...
        case 'POST':
            ...
            if ($action === 'update' && $note_id) {
                ...
                if (!mysqli_stmt_execute($stmt)) {
                    ...
                    error_log("Database error in schedule notes update: " . $db_error);
                ...
                if (!mysqli_stmt_execute($stmt)) {
                    $db_error = mysqli_error($dbc);
                    error_log("Database error in schedule notes insert/upsert: " . $db_error);
        ...
        case 'DELETE':
            ...
            if (!mysqli_stmt_execute($stmt)) {
                $db_error = mysqli_error($dbc);
                error_log("Database error in schedule notes delete: " . $db_error);
    ...
    $error_message = $e->getMessage();
 	...
 	error_log(sprintf(
        "Schedule Notes Error [User: %d, Method: %s]: %s in %s:%d",
        $actual_user_id ?? 0,
        $method ?? 'UNKNOWN',
        $error_message,
        $e->getFile(),
 	...
 	if (!in_array($error_message, $safe_messages)) {
