...
try {
    ...
    if ($action === 'create_department') {
        ...
        if (!mysqli_stmt_execute($stmt)) {
          	error_log('Create Department - Insert Error: ' . mysqli_error($dbc) . ' | Tenant ID: ' . $tenant_id . ' | Dept Name: ' . $dept_name);
        ...
        if (!mysqli_stmt_execute($stmt)) {
       	 	error_log('Create Role - Insert Error: ' . mysqli_error($dbc) . ' | Dept ID: ' . $department_id . ' | Role Name: ' . $role_name);
    ...
    mysqli_autocommit($dbc, true);
 	...
 	error_log('Create Department/Role - Exception: ' . $e->getMessage() . ' in ' . $e->getFile() . ' on line ' . $e->getLine());
    
	$safe_messages = [
        'Department name is required',
        'Department ID is required',
        'Role name is required',
        'Department not found or does not belong to this tenant',
        'Invalid action specified'
    ];
    
    if (in_array($e->getMessage(), $safe_messages)) {
      	echo json_encode([
            'success' => false,
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Failed to create ' . ($action === 'create_department' ? 'department' : 'role')
        ]);
    }
}
?>