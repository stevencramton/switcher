...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($existing['active']) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Error reactivating user assignment: ' . mysqli_error($dbc)
            ]);
        }
        exit();
    }
...
try {
    ...
    if (!mysqli_stmt_execute($stmt)) {
        throw new Exception('Error assigning user to tenant: ' . mysqli_error($dbc));
    }
	...
	if ($department_id && $job_role_id) {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            throw new Exception('Error assigning user to department role: ' . mysqli_error($dbc));
        }
    }
