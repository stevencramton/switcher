...
try {
    ...
    error_log('Update tenant order error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    
    // Return generic error message to user (security best practice)
	echo json_encode([
        'success' => false, 
        'message' => 'Unable to update tenant order. Please try again or contact support if the problem persists.'
    ]);
}
?>