...
try {
    ...
    if ($is_primary) {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            throw new Exception('Error updating primary status: ' . mysqli_error($dbc));
        }
        ...
        mysqli_stmt_bind_param($stmt, 'ii', $user_id, $tenant_id);
        if (!mysqli_stmt_execute($stmt)) {
            throw new Exception('Error setting primary tenant: ' . mysqli_error($dbc));
        }
    } else {
        ...
        mysqli_stmt_bind_param($stmt, 'ii', $user_id, $tenant_id);
        if (!mysqli_stmt_execute($stmt)) {
            throw new Exception('Error removing primary status: ' . mysqli_error($dbc));
        }
    }
    ...
    if (!empty($remove_assignments)) {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            throw new Exception('Error removing department assignments: ' . mysqli_error($dbc));
        }
    }
    ...
    foreach ($assignments as $assignment) {
        ...
        if (!mysqli_stmt_execute($stmt)) {
            throw new Exception('Error updating department role assignment: ' . mysqli_error($dbc));
        }
    }
