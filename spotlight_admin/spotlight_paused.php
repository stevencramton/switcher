...
if(isset($_POST['selected_values'])){
	...
	$query = "UPDATE spotlight_inquiry SET inquiry_status = 'Paused' WHERE inquiry_id IN ($spotlight_ids)";
	$result = mysqli_query($dbc, $query);
} else {
	$post_id = mysqli_real_escape_string($dbc, strip_tags($_POST['id']));
	$query = "UPDATE spotlight_inquiry SET inquiry_status = 'Paused' WHERE inquiry_id = '$spotlight_id'";
	$result = mysqli_query($dbc, $query);
} 
