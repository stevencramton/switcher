...
if (isset($_SESSION['id'])) {
	...
	$(document).ready(function(){
		$(".answer_move_icon").mousedown(function(){
	 		$("#sortable_create_image_select_row").sortable({
				axis: "y",
	...
	$data .='<form name="create_image_select_form" id="create_image_select_form">';
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				$inquiry_question = htmlspecialchars(strip_tags($row['inquiry_question']));
					...
					$data .='<div class="table-responsive">
						<table class="table mb-3" id="add_new_image_select_row">
								...
								<tbody class="bg-white" id="sortable_create_image_select_row">
									<tr> 
						...
						<div class="row g-2"> 
							<div class="col-md-8">
								<button type="button" class="btn btn-primary btn-lg w-100 shadow-sm" name="submit" id="create-spotlight-image-select-answers"><i class="fa-regular fa-circle-check"></i> Create Answers</button>
							</div> 
							<div class="col-md-4">
								<!-- 
								<button type="button" class="btn btn-light-gray btn-lg shadow-sm w-100 text-secondary" onclick="cancelspotlightDetails();">
									<i class="fa-solid fa-backward"></i> Close Form
								</button> 
								-->
								
								<button type="button" class="btn btn-light-gray btn-lg w-100 shadow-sm" name="add-spotlight-image-select-answer" id="add-spotlight-image-select-answer"><i class="fa-solid fa-circle-plus"></i> Row</button>
							</div>
						</div>';
...
$(document).ready(function(){ 
	...
	$('#add-spotlight-image-select-answer').click(function(){  
	...
	$('#create-spotlight-image-select-answers').click(function(){
		...
		if (response_answer_text.length === 0) {
			...
			$("#create-spotlight-image-select-answers").prop("disabled", true);
			$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Saving...');
		
			$.ajax({ 
				method:"POST",
				url: "ajax/spotlight/create_spotlight_image_radio.php",
				data: $('#create_image_select_form').serialize(), 
				type:'json',
				success:function(data){
					i = 1;
					$('.dynamic-added-row').remove();
					$('#create_image_select_form')[0].reset();
		
					$("#create-spotlight-image-select-answers").prop("disabled", false);
					$("#create-spotlight-image-select-answers").html('<i class="fa-solid fa-cloud"></i> Save');
					...
					var toastTrigger = document.getElementById("create_inquiry_spotlight_question_id")
					var toastLiveExample = document.getElementById("toast-create-spotlight-answers")
