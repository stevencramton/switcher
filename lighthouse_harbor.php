							...
							<div class="col-md-4">
								...
								<div class="upload-container p-3">
									...
									<div class="drop-zone" id="signal-drop-zone">
										<input type="file" class="file-input-hidden" id="signal-file-input" accept=".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.txt,.csv,.xls,.xlsx" multiple>
...
function createSignalRow(signal, index) {
	return `
		<tr data-signal-id="${signal.signal_id}" 
			...
			<td class="signal-files-cell" style="text-align: center;">
			    <span style="color: ${signal.attachment_count > 0 ? '#3b82f6' : '#d1d5db'};">
			        <i class="fa-solid fa-paperclip"></i> ${signal.attachment_count || 0}
			    </span>
			</td>
...
$(document).ready(function() {
	// Track TinyMCE initialization state
	var tinyMCEInitialized = false;
	
	// Initialize TinyMCE only when needed (lazy load to prevent flickering)
	...
	function switchToCreateView() {
		...
		setTimeout(function() {
			...
			setTimeout(function() {
				...
				// Initialize TinyMCE after view is visible
				initTinyMCE();
			}, 300);
		}, 300);
	}
	...
	$('#btn-send-signal').on('click', function() {
		...
		// Clear TinyMCE content after it's initialized (handled in switchToCreateView callback)
		if (tinymce.get('signal_message_content')) {
