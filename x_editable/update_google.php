...
if (isset($_POST['pk'])) {
    ...
    if (empty($value)){
        ...
        $query = "UPDATE users SET personal_email = ? WHERE id = ?";
        $stmt_update = mysqli_prepare($dbc, $query);
        mysqli_stmt_bind_param($stmt_update, "si", $value, $id);
        $result_update = mysqli_stmt_execute($stmt_update);

        if ($result_update) {
            echo "Personal Email Successfully Updated";
        } else {
            error_log("Failed to update personal email.");
            http_response_code(500);
            echo "Failed to update personal email. Please try again later.";
        }
