    ...
    if ($stmt = mysqli_prepare($dbc, $query)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            echo "Update successful.";
         	...
         	error_log('Luncheon Update - Execution Error: ' . mysqli_stmt_error($stmt) . ' | Luncheon ID: ' . $luncheon_id);
          	http_response_code(500);
            echo "Error updating luncheon.";
        }
        
        mysqli_stmt_close($stmt);
    } else {
       	error_log('Luncheon Update - Prepare Error: ' . mysqli_error($dbc) . ' | Query: ' . $query);
      	http_response_code(500);
        echo "Error updating luncheon.";
    }
}
