...
try {
        ...
        if (!empty($preferences_data)) {
            ...
            foreach ($preferences_data as $pref) {
                ...
                if (!mysqli_stmt_execute($stmt)) {
                    ...
                    error_log('Error saving preference: ' . mysqli_error($dbc));
                    // Return generic error to user
                    echo json_encode(['success' => false, 'message' => 'Error saving preference. Please try again.']);
                    exit();
                }
            }
        }
    ...
    if (!mysqli_stmt_execute($stmt)) {
        mysqli_rollback($dbc);
        // Log detailed error internally
        error_log('Error clearing existing availability: ' . mysqli_error($dbc));
        // Return generic error to user
        echo json_encode(['success' => false, 'message' => 'Error clearing existing availability. Please try again.']);
        exit();
    }
    ...
    if (!empty($availability_ranges)) {
        ...
        foreach ($availability_ranges as $avail) {
            ...
            if (!mysqli_stmt_execute($stmt)) {
                mysqli_rollback($dbc);
                // Log detailed error internally
                error_log('Error saving availability: ' . mysqli_error($dbc));
                // Return generic error to user
                echo json_encode(['success' => false, 'message' => 'Error saving availability. Please try again.']);
                exit();
            }
        }
    }
    ...
    error_log('Save user availability error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    
    // Return generic error message to user (security best practice)
    echo json_encode(['success' => false, 'message' => 'An error occurred while saving your availability. Please try again or contact support if the problem persists.']);
}
