...
function placeShiftOnTradeboard($dbc, $user_id, $input) {
    ...
    try {
        ...
        error_log('Place shift on tradeboard error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
        
        ob_end_clean();
        http_response_code(500);
        // Return generic error message to user (security best practice)
        echo json_encode(['success' => false, 'message' => 'Unable to place shift on tradeboard. Please try again or contact support if the problem persists.']);
    }
}
...
function pickupShift($dbc, $user_id, $input) {
    ...
    try {
        ...
        // Log detailed error internally for debugging
        error_log('Pickup shift error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
        
        ob_end_clean();
        http_response_code(500);
        // Return generic error message to user (security best practice)
        echo json_encode(['success' => false, 'message' => 'Unable to process trade request. Please try again or contact support if the problem persists.']);
    }
}
...
function cancelTrade($dbc, $user_id, $input) {
    ...
    try {
        ...
        // Log detailed error internally for debugging
        error_log('Cancel trade error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
        
        ob_end_clean();
        http_response_code(500);
        // Return generic error message to user (security best practice)
        echo json_encode(['success' => false, 'message' => 'Unable to cancel trade. Please try again or contact support if the problem persists.']);
    }
}
