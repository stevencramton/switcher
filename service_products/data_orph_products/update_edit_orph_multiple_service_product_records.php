...
if (isset($_POST['prod_id'])) {
	...
	$query = "UPDATE service_products SET product_name = ?, product_info = ?, product_private = ?, product_tags = ?, product_service_group = ? WHERE product_id IN ($placeholders)";
    $stmt = mysqli_prepare($dbc, $query);
    
    if ($stmt) {
   	 	$types = 'sssss' . str_repeat('i', count($prod_ids));
  	  	$params = array_merge([$types, $product_name, $product_info, $product_private, $product_tags, $product_service_group], $prod_ids);
