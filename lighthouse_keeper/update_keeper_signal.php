...
$old_query = "SELECT sea_state_id, priority_id, dock_id, service_id, keeper_assigned FROM lh_signals WHERE signal_id = ? AND is_deleted = 0";
...
try {
    ...
    if ($resolved_date !== null) {
        mysqli_stmt_bind_param($stmt, 'ssiiiiisssi', 
            $title, $message, $sea_state_id, $priority_id, $dock_id, $service_id, 
            $keeper_assigned, $resolution_notes, $current_datetime, $resolved_date, $signal_id
        );
    } else {
        mysqli_stmt_bind_param($stmt, 'ssiiiiissi', 
            $title, $message, $sea_state_id, $priority_id, $dock_id, $service_id, 
